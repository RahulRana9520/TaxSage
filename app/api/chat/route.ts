  ...
  try {
    ...
    try {
      ...
      if (userId) {
        ...
        console.log('No session userId found - user not authenticated');
      }
    } catch (error) {
...
function buildSystemPrompt(creditScore: any, creditAnalysisDate: any, userContext: UserContext | null): string {
  ...
  if (userContext && userContext.profile) {
    ...
    if (income && income.length > 0) {
      ...
      income.forEach((inc: IncomeEntry) => {
        ...
        const monthlyAmount = inc.frequency === 'monthly' ? inc.amount : 
                             inc.frequency === 'yearly' ? inc.amount / 12 : inc.amount;
    ...
    if (expenses && expenses.length > 0) {
      ...
      recentExpenses.forEach((exp: ExpenseEntry) => {
    ...
    if (budget && budget.length > 0) {
      basePrompt += `\n\nMONTHLY BUDGET PLAN:`;
      let totalBudget = 0;
      budget.forEach((allocation: BudgetAllocation) => {
    ...
    if (goals && goals.length > 0) {
      ...
      goals.forEach((goal: Goal) => {
        ...
        const monthsToGoal = Math.ceil((targetDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24 * 30));
